...
<script>
    ...
    export default {
        ...
        data:()=>({
          ABflag: "1", //类型
          navbarList:[{name:"A版",type:1},{name:"B版",type:2}],
          isEdit: true,  //是否编辑
          ...
          delStyle:{background: '#FF1E41', color: '#fff',lineHeight: '1.22rem'},
          productList:[],
          settlement:{
            num: 1
          },
        ...
        methods:{
          ...
          del(all,item){
            ...
            //删除单条
            if(all == false){
              let {goodsId, goodsSpecId, goodsPhId} = item;
              goodsIds = [{goodsId, goodsSpecId, goodsPhId}];
            }else{
              //是否能结算
              ...
              this.productList.forEach((item)=>{
                if(item.goodsSelected == 1){
                  let {goodsId,goodsSpecId,goodsPhId} = item;
                  goodsIds.push({goodsId,goodsSpecId,goodsPhId})
                }
              })
            }
            this.$Indicator.loading();
            let url = "/UserInterface/cart/delGoods.ashx";
            let param = {goodsId: JSON.stringify(goodsIds),ABflag:this.ABflag}
            this.$post(url,param).then((data)=>{
              this.renderHtml(data)
              this.$Indicator.close();
            })
          ...
          allSelect(){
            ...
            //2  选中 3  取消
            let param = {goodsId:"", goodsSpecId:"", goodsPhId:"" ,ABflag:this.ABflag,selectState };
            this.$post(url,param).then((data)=>{
              this.renderHtml(data)
              this.$Indicator.close();
            })
...
<style scoped lang="scss">
  ...
  .settlement{
    .bar{
        ...
        .check-i,.allMoney,.btn-buy, .check-area span{
          display: inline-block;
          vertical-align: middle;
        }
        .right{
        ...
        .allMoney{
          ...
          margin-right: 0.1rem;
          color: $color60;
          span{font-size: 0.16rem;color: $themeColor}
        }
    }
  }
</style>
