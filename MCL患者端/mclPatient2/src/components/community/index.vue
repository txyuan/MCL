<template>
  <div class="community-root">
          ...
          <div slot="content">
            <div class="content" v-show="ABflag == 1">
              <router-link tag="div" to="/edition">
                <div class="education marginTop10">
                  <div class="title font16"> <span>在线教育</span> </div>
                  <img src="@/assets/images/位图@2x.png" width="100%" />
                </div>
              </router-link>

              <div
                class="chart marginTop10"
                v-for="(item, index) in dynamicList"
                :key="index"
              >
                <div class="title font16" v-if="index == 0">
					<span class="name">所有动态</span>
					<div class="types" @click="showPanel = true">热门</div>
				</div>
                <mt-cell>
                ...
                <div class="bar clear">
                  <div
                    class="xin"
                    :class="{ active: item.followStatus == 1 }"
                    @click="doFollow(item)"
                  >
                    {{ item.followCount }}
                  </div>
                  ...
                  <div
                    class="xing"
                    :class="{ active: item.praiseStatus == 1 }"
                    @click="doPraise(item)"
                  >
                    {{ item.praiseCount }}
                  </div>
            ...
            <div class="content" v-show="ABflag == 2">
              <router-link
                ...
                :key="index"
                tag="div"
                :to="`/activetyDetail?id=${item.acskey}`"
              >
                <div class="activity marginTop10">
                  <div class="item margin10">
                    ...
                    <div class="activity-bar font12 huiFont">
                      <div>
                        <img
                          src="@/assets/images/yijieshu@2x.png"
                          class="icon"
                          width="15"
                        />
            ...
            <div class="content" v-show="ABflag == 3">
              <div class="activity marginTop10">
                <div
                  ...
                  :key="index"
                  @click="$router.push(`/caseDetail?id=${item.caseSkey}`)"
                >
                  ...
                  <div class="font12 huiFont">
                    ...
                    <div class="des">
                      ...
                      <div class="seeNum">
                        <img src="@/assets/images/eyeh.png" alt="" />
                ...
                <mt-cell>
                  ...
                  <div class="font14 huiFont99">
                    <mt-badge
                      size="normal"
                      color="#F78335"
                      @click.native="delDynamic(item, index)"
                      >删除</mt-badge
                    >
                    <mt-badge size="normal" color="#888" v-if="item.state == 1"
                      >审核中</mt-badge
                    >
                    <mt-badge
                      size="normal"
	...
	<transition name="fade">
		<div id="mark" style="display: block;z-index: 99999" v-show="showPanel" @click="showPanel = false">
			<div class="labels" @click.stop="">
        <img src="@/assets/images/guanbi@2x.png" alt="" class="close" @click="showPanel = false">
				<span v-for="(item, index) in labels" :key="index" :class="{active: item.flag == 1}" @click.stop="item.flag = (item.flag == 1 ? 0 : 1)">{{item.LableName}}</span>
        <mt-button type="primary" class="theme-button" size="small" @click.native="saveLabel">保存</mt-button>
			</div>
		</div>
	</transition>
...
<script>
import { getCommunityLabelList, saveLable } from "@/api/community.js"
  ...
  methods: {
    ...
    doPraise(item) {
      ...
      this.$post(url, param).then((data) => {
        ...
        item.praiseCount =
          praiseStatus == 1 ? (praiseCount += 1) : (praiseCount -= 1);
    ...
    doFollow(item) {
      ...
      this.$post(url, param).then((data) => {
        ...
        item.followCount =
          followStatus == 1 ? (followCount += 1) : (followCount -= 1);
    ...
    // 获取热门动态
    async getCommunityLabelList(){
      const {data} = await getCommunityLabelList()
      const values = data.filter(item => item.flag == 1)
      if(values.length == 0){
        this.showPanel = true
      }
      this.labels = data
    ...
    // 保存标签
    async saveLabel(){
      const values = this.labels.filter(item => item.flag == 1)
      const skeys = values.map(item => item.sKey)
      const data = await saveLable({skey: skeys.join(",")})
      if(data.rspcode == 0){
        this.$Toast(data.rspdesc)
      }else{
...
.title .types{
	...
	font-weight: normal;
	font-size: 0.14rem;
	background: url('../../assets/images/Shape@2x.png') no-repeat right 0.15rem;
