  ...
  <div class="home">
    ...
    <div>
      <div class="gauge_3">
        <p>6、大腿（四头肌）</p>
        <div class="images">
          <img src="@/assets/images/大腿.png" alt="" />
        </div>
        <van-radio-group
          class="one_gau_radio"
          v-model="data.attribute_value_46"
          direction="horizontal"
        >
          <van-radio
            v-for="(item, i) in list"
            :key="i"
            :class="{ checkedOne: data.attribute_value_46 == item.value }"
            :name="item.id"
  ...
  watch: {
    aaa: {
      ...
      handler(newVal, oldVal) {
        if (newVal.attribute_value_46 != "") {
          let nutrition = this.$store.state.nutrition;
          let obj12 = {
            ...nutrition,
            ...this.data,
          };
          this.$store.commit("setnutrition", obj12);
          setTimeout(() => {
            this.$router.push("/nutrition13");
          }, 500);
        }
      },
      deep: true,
