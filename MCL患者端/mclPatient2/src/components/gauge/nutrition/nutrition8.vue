<template>
  <div class="home">
    ...
    <div>
      <div class="gauge_3">
        <p>2、锁骨部位（胸部三角肌） ：看锁骨是否凹陷</p>
        <div class="images">
          <img src="@/assets/images/锁骨部位.png" alt="" />
        </div>
        <van-radio-group
          class="one_gau_radio"
          v-model="data.attribute_value_42"
  ...
  watch: {
    aaa: {
      ...
      handler(newVal, oldVal) {
        if (newVal.attribute_value_42 != "") {
          let nutrition = this.$store.state.nutrition;
          let obj8 = {
            ...nutrition,
            ...this.data,
          };
          this.$store.commit("setnutrition", obj8);
          setTimeout(() => {
            this.$router.push("/nutrition9");
          }, 500);
        }
      },
      deep: true,
