        ...
        <van-checkbox-group v-model="result4">
          <van-cell-group :border="false">
            ...
            >
             <img class="left_img" :src="item.ImageUrl" alt="">
             <template #right-icon >
        <van-checkbox :name="item" ref="checkboxes4" />
        <span class="right_img"></span>
      </template>
            </van-cell>
  ...
  watch: {
    'result1': {
        handler(newValue, oldValue) {
            if (newValue.length == 0 && this.result2.length == 0 && this.result3.length == 0 && this.result4.length == 0) {
              // this.result = []
                // 操作
                this.valueAll = false
            }else {
    ...
    'result2': {
        handler(newValue, oldValue) {
            if (newValue.length == 0 && this.result1.length == 0 && this.result3.length == 0 && this.result4.length == 0) {
              // this.result = []
                // 操作
                this.valueAll = false
            }else {
    ...
    'result3': {
        handler(newValue, oldValue) {
            if (newValue.length == 0 && this.result2.length == 0 && this.result1.length == 0 && this.result4.length == 0) {
              // this.result = []
                // 操作
                this.valueAll = false
            }else {
    ...
    'result4': {
        handler(newValue, oldValue) {
            if (newValue.length == 0 && this.result1.length == 0 && this.result2.length == 0 && this.result3.length == 0) {
              // this.result = []
                // 操作
                this.valueAll = false
            }else {
  ...
  methods: {
    goInfo() {
      ...
      // let nutrition = this.$store.state.nutrition;
          ...
          //   ...this.data,
          // };
          this.$router.push(`/symptom${arr[0].sKey}`)
          // arr.shift()
          this.$store.commit("setsymptom", arr);
    },
     toggle1(item,index){
...
.content {
  ...
  >>> .van-cell-group {
    ...
    .active {
      ...
      box-shadow: 0 2px 2px 0 rgba(0,0,0,0.14), 0 3px 1px -2px rgba(0,0,0,0.1), 0 1px 10px 0 rgba(0,0,0,0.12);
      .right_img {
        position: absolute;
        right: 0;
        bottom: 0;
