      ...
      <div class="main"
        ...
        <ul style="overflow: hidden;  -webkit-transform: translateX(40px);">
          <li v-touch:touchstart="onTouchStart" v-touch:touchmove="onTouchMove" v-touch:touchend="onTouchEnd"
              v-touch:touchcancel="onTouchEnd" v-for="(n,index) in itemCount" :key="n"><span class="num">{{min  + index }} </span>
          </li>
        </ul>
      </div>
      <div class="arrow"></div>
...
<script>
  export default {
    ...
    computed: {
      ...
      getMainWidth () {
        return ((this.max - this.min) / 1+ 2) * 104
      }
    },
    created () {
    },
...
<style>
  ...
  .ymRulerLiang .rultitle {
    text-align: center;
    margin-top: 0.15rem;
    margin-bottom: 0.2rem;
  }
  .ymRulerLiang .rultitle strong{
    font-weight: 600;
    font-size: 0.275rem;
  ...
  .ymRulerLiang .ruler .rulltbg, .ymRulerLiang  .ruler .rulrtbg{
    position: absolute;
    width: 15%;
    top: -30px;
    bottom: 0;
