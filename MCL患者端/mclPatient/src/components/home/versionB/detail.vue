<template>
  <div class="paddimg">
    ...
    <div class="detile_lunbo">
      <img v-lazy="swiperArr[0]" alt="" width="100%" :height="screenWidth" style="display: none;">
    ...
    <div class="detile_shopname">
      ...
      <div class="detile_money">
				<!-- <div class="detile_prices">市场价¥{{goodsInfo.price}}</div> -->
				<span style="font-size: 0.16rem;width: 100%;margin-top: 0.04rem;color: #333;"><u style="font-size: 0.14rem;text-decoration: none;" v-if="goodsInfo.ProDistinguish=='03'">抢购价：</u><u style="font-size: 0.14rem;text-decoration: none;" v-else>市场价：</u>¥{{goodsInfo.marketPrice}}</span>
				<span v-if="goodsInfo.ProDistinguish!='03'" style="font-size: 0.16rem;margin-top: 0.04rem;display: block;width: 100%;"><u style="font-size: 0.14rem;text-decoration: none;">会员价：</u>¥{{goodsInfo.price}}+{{goodsInfo.BSpendingTicket}}券</span>
        <div class="detile_price">快递费 :{{goodsInfo.ExpressCost}}</div>
				<!-- <span v-if="goodsInfo.score==0">¥{{goodsInfo.price}}</span>
        <span v-else>¥{{goodsInfo.price}}+{{goodsInfo.score}}积分</span> -->
        <!-- <label style="margin-right: 0.22rem">库存：{{goodsInfo.inventory}}</label> -->
        <div class="clear"></div>
      </div>
    </div>
    ...
    methods:{
      ...
      goshop(){
				...
				this.$post(url,param).then((data)=>{
					...
					this.$router.push({path:"/orderindex",query: { orderFrom: 0, ABflag:2 }})
				})
      ...
      //获取详情信息
      getDetailInfo(sKey){
        let url = "/UserInterface/user/getProductBDetails.ashx";
