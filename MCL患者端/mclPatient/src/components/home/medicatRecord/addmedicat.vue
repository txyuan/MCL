	...
	<div>
		...
		<div class="medicat_home">
			<div class="medicat_name">
				<p>药品名称: </p>
				<input type="text" v-model="param.drug_name" />
			</div>
			...
			<div class="medicat_yaol">
				<div class="medicat_yaoever">
					<!-- <span>每次用量：</span> -->
					<mt-cell title="每次用量：" is-link @click.native="pickerToggle('show')" style="border-bottom: 1px solid #eee;font-size: 0.12rem;">
				...
				<div class="medicat_yaoever">
					...
					<mt-cell title="剂量：" is-link @click.native="pickerToggles('show')" style="border-bottom: 1px solid #eee;font-size: 0.12rem;">
			...
			<div class="medicat_pingl">
				...
				<ul>
					<li>
						<span>第一次</span>
						<mt-cell title="" is-link @click.native="pickerToggles1('show')" style="border-bottom: 1px solid #eee;width: 25%;float: left;">
						...
						<div class="medicat_eatbf">
							<p v-for="(item,index) in list1" :key="index" :class="{active:index==current1}" @click="getdefalt1(index)">
								<i>{{item.names}}</i>
								<label>
									<u></u>
								</label>
					...
					<li>
						...
						<mt-cell title="" is-link @click.native="pickerToggles2('show')" style="border-bottom: 1px solid #eee;width: 25%;float: left;">
							<span>{{param.second_time}}</span>
						</mt-cell>
						<div class="medicat_eatbf">
							<p v-for="(item,index) in list1" :key="index" :class="{active:index==current2}" @click="getdefalt2(index)">
								<i>{{item.names}}</i>
								<label>
									<u></u>
								</label>
					...
					<li>
						...
						<mt-cell title="" is-link @click.native="pickerToggles3('show')" style="border-bottom: 1px solid #eee;width: 25%;float: left;">
							<span>{{param.third_time}}</span>
						</mt-cell>
						<div class="medicat_eatbf">
							<p v-for="(item,index) in list1" :key="index" :class="{active:index==current3}" @click="getdefalt3(index)">
								<i>{{item.names}}</i>
								<label>
									<u></u>
								</label>
			...
			<div class="medicat_styl">
				<span>不良反应 : </span>
				<div class="medicat_eatbf">
					<p style="width: 70%;" v-for="(item,index) in listc" :key="index" :class="{active:index==current6}" @click="getdefalt6(index)">
						<label>
							<u></u>
						</label>
						<i>{{item.names}}</i>
						<input type="text" v-if="item.names=='有'" v-model="param.side_effects_result" style="border-bottom: 1px solid #999;float: left;width: 66%;margin-left: 0.04rem;" />
					</p>
				</div>
			</div>
			<div class="medicat_styl">
			...
			<p class="medicat_btn" @click="setpost">确定并增加</p>
		</div>
		<!-- 每日用药picker  -->
		...
		<mt-popup v-model="popupVisibles1" position="bottom">
			<mt-picker :slots="sexSlotses1" :showToolbar="true" :visibleItemCount="3" ref="sexPickers1" @change="visbleChange">
				<div class="picker_bar">
		...
		<mt-popup v-model="popupVisibles2" position="bottom">
			<mt-picker :slots="sexSlotses2" :showToolbar="true" :visibleItemCount="3" ref="sexPickers2" @change="visbleChange">
				<div class="picker_bar">
		...
		<mt-popup v-model="popupVisibles3" position="bottom">
			<mt-picker :slots="sexSlotses3" :showToolbar="true" :visibleItemCount="3" ref="sexPickers3" @change="visbleChange">
				<div class="picker_bar">
