...
export default {
  ...
  data: () => ({
    ...
    sportSlot: [{
      flex: 1,
      values: [{name: '肿瘤管理', id: '01'}, {name: '健康管理', id: '02'}, {name: '体重管理', id: '03'}],
      className: 'slot1',
      textAlign: 'center'
    }]
  }),
  methods: {
    ...
    addSubmit () {
      ...
      if (this.diseaseSubmit()) {
        ...
        if (diseasetype === '01') {
          ...
          Object.assign(param, zlTab.param)
        } else if (diseasetype === '02') {
          url = 'UserInterface/AddConditionDiseaseDisease.ashx'
          Object.assign(param, healthTab.param)
        } else if (diseasetype === '03') {
          url = 'UserInterface/AddConditionDiseaseWeight.ashx'
          Object.assign(param, weightTab.param)
        }
    ...
    editSubmit () {
      ...
      if (this.diseaseSubmit()) {
        let url = ''
        if (diseasetype === '01') {
          url = 'UserInterface/UpdateConditionDiseaseTumour.ashx'
          Object.assign(param, zlTab.param)
        } else if (diseasetype === '02') {
          url = 'UserInterface/UpdateConditionDiseaseDisease.ashx'
          Object.assign(param, healthTab.param)
        } else if (diseasetype === '03') {
          url = 'UserInterface/UpdateConditionDiseaseWeight.ashx'
          Object.assign(param, weightTab.param)
        }
        this.$post(url, param).then((data) => {
          if (data.rspcode != 1) {
    ...
    diseaseSubmit () {
      ...
      if (diseasetype === '01') {
        return zlTab.verification()
      } else if (diseasetype === '02') {
        return healthTab.verification()
      } else if (diseasetype === '03') {
        return weightTab.verification()
      }
      return true
    },
    ...
    getDetail () {
      ...
      // 01：肿瘤，02：健康 03：体重
      if (diseasetype === '01') {
        url = 'UserInterface/SelectConditionDiseaseTumour.ashx'
      } else if (diseasetype === '02') {
        url = 'UserInterface/SelectConditionDiseaseDisease.ashx'
      } else if (diseasetype === '03') {
        url = 'UserInterface/SelectConditionDiseaseWeight.ashx'
      }
      this.$post(url).then((data) => {
        ...
        // 设置管理类型默认值
        const { sportPickers, weightTab, healthTab, zlTab } = this.$refs
        this.sportSlot[0].values.forEach((item, index) => {
          if (item.id === diseasetype) {
            // 设置默认的管理类型
            sportPickers.setSlotValue(0, item)
            this.sportConfirm()
            this.$nextTick(() => {
              this.openTouch()
            })
        ...
        // 设置表单值，管理类型的三个tab（体重管理，健康管理，肿瘤管理）
        // 01：肿瘤，02：健康 03：体重
        if (diseasetype === '01') {
          zlTab.setFormData(data)
        } else if (diseasetype === '02') {
          healthTab.setFormData(data)
        } else if (diseasetype === '03') {
          weightTab.setFormData(data)
        }
      })
    }
  },
  ...
  mounted () {
    ...
    if (this.$route.query.type === 'look') {
      this.param.diseasetype = this.$route.query.diseasetype
      this.getDetail()
    } else {
