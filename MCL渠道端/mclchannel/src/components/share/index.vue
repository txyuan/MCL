	...
	<div style="background: #fff;">
		...
		<div :style="{height:heigt+'px'}" class="share">
			<div class="share_cont">
				<div class="share_top">
					<h4>{{UserName}} <span v-if="Department || City">（<i>{{Department}}</i><i v-if="Department && City" style="margin:0 5px">|</i><i>{{City}}</i>）</span></h4>
					<p>
						<label>邀请码</label>
						<i>{{InvitationCode}}</i>
					</p>
				...
				<div class="share_bot" v-show="isShowBtn">
					<span @click="shareclick"><img src="@/assets/images/fenxiang@2x.png" class="icon"/>分享</span>
					<!--<label>保存到相册</label>-->
				</div>
			</div>
		</div>
...
<script>
	...
	export default {
		...
		mounted() {
			...
			//返回按钮  和 分享按钮的显示
			if(this.$route.query.title == "邀请好友"){
