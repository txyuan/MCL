        ...
        methods:{
          ...
          change(e,type){
            ...
            if(type == 'A'){
              ...
              this.param.idcardgs = url;
              axios.post('UserInterface/UploadFile.ashx', params, config).then(response => {
                if (response.data.rspcode != 1) {
                  return;
                }
                this.param.idcardgs=response.data.url;
              })
            }
          },
          changes(types){
          ...
          showbg(name){
            ...
            this.$post(url,{BinaryStream: name}).then((data) => {
              ...
              if(this.pdsfz==1){
                ...
                this.isShowDefalutIdCardC = false;
                this.param.idcardgs=data.data.imgUrl;
              }
            })
          },
          //显示性别picker
					...
					showInform(){
						...
						this.$post(url).then((data)=>{
						  ...
						  this.param.idcardjust=data.data.idCardJust;
							...
							this.param.bankkey=data.data.validity_time;
						})
					},
          //编辑个人信息
          editUserInfo(){
