<template>
  <div class="real_name_root padding-header">
    <mt-header title="修改银行卡" fixed class="borderBottom">
      <div slot="left">
        <header-back>
    ...
    <div class="main">
      ...
      <div class="form" style="margin-top: 0.06rem">
        ...
        <mt-field label="真实姓名：" v-model.trim="param.user_name" readonly type="text" class="borderBottom" placeholder="请输入真实姓名"></mt-field>
        <mt-field label="身份证号：" v-model.trim="param.id_card" readonly type="text" class="borderBottom" placeholder="请输入身份证号"></mt-field>
        <mt-field label="银行卡号：" v-model.trim="param.bank_card" type="number" class="borderBottom" placeholder="请输入银行卡号"></mt-field>
        <!--<mt-field label="发卡银行：" v-model.trim="param.bankkey" type="text" class="borderBottom" placeholder="请输入发卡银行"></mt-field>-->
        <mt-cell title="发卡银行：" is-link @click.native="pickerToggle('show')" style="border-bottom: 1px solid #eee">
          <span>{{param.bankkey}}</span>
        </mt-cell>
  ...
  export default {
    ...
    data: () => ({
      ...
      param:{
        ...
        bank_card:"",  //银行卡号
        bankkey:"",  //发卡银行
        realname:2   //判断实名认证还是银行卡，1、实名  2、银行卡
      },
      pdsfz:1,  //判断身份证正反面
      anios:1,
      repData:{
      ...
      sexSlots: [
        {
          flex: 1,
          values: ['中国工商银行', '中国农业银行','中国银行','中国建设银行','交通银行','中信银行','中国光大银行','华夏银行','中国民生银行','广发银行','深圳发展银行','招商银行','兴业银行','上海浦东发展银行','恒丰银行','浙商银行','渤海银行','中国邮政储蓄银行','北京银行'],
          className: 'slot1',
          textAlign: 'center'
        }
      ],
    ...
    methods:{
      ...
      //反显信息
      getname(){
        let url = "UserInterface/RealNameInfo.ashx";
        this.$post(url).then((data) => {
          if(data.rspcode==0){
            this.$Toast(data.rspdesc)
            return;
          }
          this.param.user_name=data.real_name;
          this.param.id_card=data.id_number;
        })
      },
      change(e,type){
      ...
      submit(){
        ...
        let url = "UserInterface/RealNameAuthentication.ashx";
...
<style scoped lang="scss">
  ...
  .img_wrap{
    ...
    .img .defaultImg{
      height: 100%;
      img{width: 18%;margin-top: 50px;margin-bottom: 25px;}
    }
    .img input[type=file]{
      opacity: 0;
      width: 100%;
